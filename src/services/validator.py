...
class Validator:
    ...
    def validate_book(self,info):
        ...
        if info["volume"] != "" and re.match("^\d+$", info["volume"]) is None:
            errors.append("Volume should contain only numbers")

        if info["month"] != "" and re.match("^(0?[1-9]|1[0-2])$", info["month"]) is None:
    ...
    def validate_manual(self,info):
        ...
        if info["author"] != "" and re.match("[a-zA-Z]+$", info["author"]) is None:
            errors.append("Author should contain only letters")

        if info["year"] != "" and re.match("^\d+$", info["year"]) is None:
            errors.append("Year should contain only numbers")
